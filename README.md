...
> **Node 2.0 context menu branch:** This version uses ComfyUI's current `getNodeMenuItems` extension API instead of deprecated `getNodeMenuOptions` monkey-patching. Node right-click menus are intentionally quick-only: quick-map node mute/bypass plus quick map/unmap entries for mappable widgets present on the node. It does not add the old Standard Map submenus to the node context menu.

- **Versatile Mapping:** Map any controller input (buttons, axes, knobs, sliders) to:
    - Any node parameter (widgets like sliders, toggles, number inputs, dropdowns) via quick mapping from the node context menu or advanced mapping from the Controller Panel.
...
- **Quick Mapping:** Instantly map a controller input to a widget using default settings via node right-click context menu entries for each mappable widget.
- **Quick Node State Mapping:** Right-click a node and choose `ControlFreak: Quickmap Mute` or `ControlFreak: Quickmap Bypass`. These mappings are direct by default, so MIDI toggle buttons that alternate 127/0 turn mute/bypass on and off.
...
- **Right-click anywhere on a node:** The Node 2.0 context menu lists quick-map entries for mappable widgets present on that node, such as `ControlFreak: Quick Map 'strength_model'`.
- **Select the widget entry:** Choose the widget/value you want to control.
...
**2. Quick Map Node State:**

- **Right-click anywhere on a node:** Choose `ControlFreak: Quickmap Mute` or `ControlFreak: Quickmap Bypass`.
- **Move Controller:** Activate the desired controller input.
- **MIDI toggle behavior:** For MIDI buttons that alternate 127/0, node-state quickmaps are direct by default: 127 enables mute/bypass, 0 restores the node.

**3. Advanced Mapping (For more configuration):**

Use the Controller Panel for advanced mapping options such as custom ranges, mapping type changes, reassignment, and deleting multiple mappings. The Node 2.0 right-click context menu is intentionally quick-only and does not add the old `Standard Map...` submenu.

<p align="center">
  <img src="assets/learning.png" alt="Controller Learning Process" width="700"/>
</p>

**4. Quick Unmap (If *only one* mapping exists):**

- **Right-click the node:** If a widget has exactly one mapping, the node context menu will show `ControlFreak: Quick Unmap '[Widget Name]'`.
- **Select the unmap entry:** The single mapping is instantly removed, and the highlight disappears. A notification confirms the unmapping.
